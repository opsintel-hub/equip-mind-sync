...
interface MediaPlayerCodePrefix {
  id: string;
  prefix: string;
...
interface MediaPlayerCodePrefixSelectProps {
  value: string;
...
export function MediaPlayerCodePrefixSelect({ value, onChange, disabled, onCodeGenerated }: MediaPlayerCodePrefixSelectProps) {
  const [prefixes, setPrefixes] = useState<MediaPlayerCodePrefix[]>([]);
  ...
  const fetchPrefixes = async () => {
    const { data, error } = await supabase
      .from("media_player_code_prefixes")
      .select("*")
      .eq("is_active", true)
      .order("prefix");

    if (error) {
      console.error("Error fetching media player prefixes:", error);
  ...
  const requestDelete = async (p: MediaPlayerCodePrefix) => {
    setDeleteId(p.id);
    setUsage(null);
    try {
      setUsage(await checkPrefixUsage("media_player", p.prefix));
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    if (!target) return;
    try {
      const u = usage ?? (await checkPrefixUsage("media_player", target.prefix));
      const inUse = u.count > 0;
      await removeCodePrefix("media_player", target.id, !inUse);
    ...
    <>
      <div className="flex gap-2">
        <div className="flex-1">
          <Select value={value} onValueChange={onChange} disabled={disabled}>
            <SelectTrigger>
              <SelectValue placeholder="เลือก Prefix รหัส Media Player" />
            </SelectTrigger>
      ...
      <Dialog open={isManageOpen} onOpenChange={setIsManageOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>จัดการ Prefix รหัส Media Player</DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              กำหนด Prefix ได้ 1-7 ตัวอักษร เช่น MP, MDP, LED ระบบจะเติมเลขรัน 4 หลักให้อัตโนมัติ (เช่น MP 0001)
            </p>
            ...
            {!isAdding ? (
              ...
              <div className="border rounded-lg p-4 space-y-3">
                <Input
                  placeholder="Prefix (1-7 ตัวอักษร) เช่น MP"
                  value={newPrefix}
