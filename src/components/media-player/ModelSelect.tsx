...
export function ModelSelect({ value, onChange, disabled }: ModelSelectProps) {
  ...
  const handleSave = async () => {
    if (!formData.name.trim()) { toast.error("กรุณากรอกชื่อ Model"); return; }
    setIsLoading(true);
    try {
      if (editingItem) {
        const { error } = await supabase.from("media_player_models")
          .update({ name: formData.name, description: formData.description || null })
          .eq("id", editingItem.id);
        if (error) throw error;
        toast.success("แก้ไข Model สำเร็จ");
      } else {
        const { error } = await supabase.from("media_player_models")
          .insert({ name: formData.name, description: formData.description || null });
        if (error) throw error;
        toast.success("เพิ่ม Model สำเร็จ");
      }
    ...
    <div className="flex gap-2">
      <div className="flex-1">
        <SearchableSelect
          ...
          searchPlaceholder="ค้นหา Model..."
          emptyMessage="ไม่พบ Model"
          disabled={disabled}
        />
      </div>
        ...
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="กรอกชื่อ Model" />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button onClick={handleSave} disabled={isLoading} className="flex-1">
                ...
                {editingItem ? "บันทึกการแก้ไข" : "เพิ่ม Model"}
              </Button>
              {editingItem && (
            ...
            <div className="border-t pt-4">
              <Label className="mb-2 block">Model ที่มีอยู่</Label>
