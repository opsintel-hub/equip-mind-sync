...
ALTER TABLE public.media_players ADD COLUMN IF NOT EXISTS sub_media_type TEXT;

CREATE OR REPLACE FUNCTION public.validate_mp_sub_media_type()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $$
DECLARE
  v_allowed TEXT[] := ARRAY['TOPSHELF_1','TOPSHELF_2','TOPSHELF_3','SPECIAL_1','SPECIAL_2','OVERVAULT_1','OVERVAULT_2','OPENTYPE_1','OPENTYPE_2'];
  v_is_7eleven BOOLEAN;
BEGIN
  v_is_7eleven := NEW.department IS NOT NULL AND lower(btrim(NEW.department)) = lower('7-Eleven Media');

  IF v_is_7eleven THEN
    IF NEW.sub_media_type IS NULL OR btrim(NEW.sub_media_type) = '' THEN
      RAISE EXCEPTION 'ฝ่าย 7-Eleven Media ต้องระบุ Sub Media Type (TOPSHELF_1..OPENTYPE_2)';
    END IF;
    IF NOT (NEW.sub_media_type = ANY(v_allowed)) THEN
      RAISE EXCEPTION 'Sub Media Type ไม่ถูกต้อง: %', NEW.sub_media_type;
    END IF;
  ELSE
    NEW.sub_media_type := NULL;
  END IF;
...
DROP TRIGGER IF EXISTS validate_mp_sub_media_type_trigger ON public.media_players;
CREATE TRIGGER validate_mp_sub_media_type_trigger
BEFORE INSERT OR UPDATE OF department, sub_media_type ON public.media_players
FOR EACH ROW EXECUTE FUNCTION public.validate_mp_sub_media_type();

-- Update import RPC to accept sub_media_type
...
BEGIN
  ...
  -- Sub media type validation (also enforced by trigger; check here for nicer error)
  ...
  INSERT INTO public.media_players (
    ...
    billboard_id, install_date, status, sub_media_type,
