...
DECLARE
  ...
  item_tables text[] := ARRAY[
    ...
    'technician_tools','tool_pm_types',
...
BEGIN
  ...
  WITH RECURSIVE dep AS (
    SELECT (x COLLATE "default") AS tbl FROM unnest(target) x
    UNION
    ...
    JOIN dep d ON d.tbl = (p.relname::text COLLATE "default")
  ...
  IF array_length(cascaded, 1) IS NOT NULL THEN
    RAISE EXCEPTION 'scope_incomplete: การลบตามตัวเลือกที่เลือกจะทำให้ตารางอื่นถูกลบตามไปด้วย (%) กรุณาเลือกกลุ่มข้อมูลเพิ่มให้ครบก่อน', array_to_string(cascaded, ', ');
  END IF;
