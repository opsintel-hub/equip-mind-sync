...
export default function PMHistory() {
  ...
  const distinctDepts = [...new Set(historyData.map(r => r.billboards?.department).filter(Boolean))].sort() as string[];
  const distinctMediaTypes = [...new Set(historyData.map(r => r.billboards?.media_type).filter(Boolean))].sort() as string[];
  ...
  const hasActiveFilters = filterDept !== "all" || filterMediaType !== "all" || filterPmReason !== "all" || filterAction !== "all" || !!dateRange;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">ประวัติการดำเนินการ PM ป้ายโฆษณาทั้งหมด (Magic Ticket) พร้อมสถิติ</p>
      </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Select value={filterMediaType} onValueChange={setFilterMediaType}>
              ...
              <SelectContent>
                ...
                {distinctMediaTypes.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}
              </SelectContent>
            </Select>
